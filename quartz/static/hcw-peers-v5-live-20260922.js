...
(function () {
  if (window.__hcwPeersV5Live20260922Installed) return;
  window.__hcwPeersV5Live20260922Installed = true;

  const current = document.currentScript?.src || new URL("static/hcw-peers-v5-live-20260922.js", document.baseURI).href;
  const dataUrl = new URL("hcw-peers-v5.json", current).href;
  const reduce = () => matchMedia("(prefers-reduced-motion: reduce)").matches;

  const liveCssUrl = new URL("hcw-peers-v5-live-20260922.css", current).href;
  if (!document.querySelector('link[data-hcw-peers-v5-live]')) {
    const liveStyle = document.createElement("link");
    liveStyle.rel = "stylesheet";
    liveStyle.href = liveCssUrl;
    liveStyle.dataset.hcwPeersV5Live = "1";
    document.head.appendChild(liveStyle);
  }
  console.info("[H.cw] peers-v5-live-20260922 loaded");
